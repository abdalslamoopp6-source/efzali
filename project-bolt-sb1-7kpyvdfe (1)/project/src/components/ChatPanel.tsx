    ...
    <div className="flex flex-col h-72 rounded-xl border border-slate-700 bg-slate-800/30 overflow-hidden">
          ...
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isMine ? 'justify-start' : 'justify-end'}`}>
                <div className={`max-w-[75%] rounded-2xl px-3.5 py-2 ${isMine ? 'bg-amber-500/20 text-amber-100 rounded-tl-sm' : 'bg-slate-700 text-slate-100 rounded-tr-sm'}`}>
      ...
      <form onSubmit={handleSend} className="flex items-center gap-2 border-t border-slate-700/50 bg-slate-800/50 px-3 py-2.5">
        <input
          ...
          disabled={sending}
          className="flex-1 rounded-xl border border-slate-600 bg-slate-900/50 px-3 py-2 text-sm text-white placeholder-slate-500 outline-none transition-colors focus:border-amber-500/50"
        />
        <button
          type="submit"
          disabled={!input.trim() || sending}
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-l from-amber-500 to-orange-600 text-white transition-transform hover:scale-105 disabled:opacity-50 disabled:scale-100"
        >
